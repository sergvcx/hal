...
{
    if ( PL_ReadMemBlock( a, (PL_Word*)bsStatus, datSecAddr, bobSize) != PL_OK )
...
{
    if ( PL_WriteMemBlock( a, (PL_Word*)bsStatus +bobSizeIn, 
                                    datSecAddr +bobSizeIn, 
                                    bobSize -bobSizeIn) != PL_OK )
...
size_t
charToSparse( PL_Word* dst, char* src, size_t count ){
    size_t res= 0;
    size_t i;
    if ( count > backBufferSize )
        throw "host-to-board buffer overflow";
    for ( i=0; i<count; i++ ){
        PL_Word c= *dst++= (PL_Word)(unsigned char)*src++;
        res++;
        //if ( c==0 )
        //    break;
    }
...
NM_IO_Service::NM_IO_Service( const char* fName, PL_Access* access, 
...
NM_IO_Service::dispatch()
{
    ...
    bool proceed= true;
    PL_Word bufferToBoard[ backBufferSize ];
    set<int> isTextMode;
    while ( proceed )
    {
	    Sleep(10);
        ...
        if ( sended< sendConfirmed ){
            ...
            assert (words>=0);
            if ( words ){
                r= PL_ReadMemBlock( a, (PL_Word*)hostBuf, sendConfirmed, words);
                if ( r!= PL_OK )
        ...
        if (words){
            ...
            while  ( hostReceive <boardSend ){
                ...
                case INDEX_GETCHAR:
                    ...
                    backBufferReady= 1;
                    bufferToBoard[0]= (PL_Word)getchar(); 
                    break;
                case INDEX_OPEN:
                    assert( backBufferReady ==0 );
                    {
                        ...
                        int fd= (PL_Word)_open( charBuf, mode, _S_IREAD | _S_IWRITE ); 
                ...
                case INDEX_CLOSE:
                    assert( backBufferReady ==0 );
                    backBufferReady= 2;
                    bufferToBoard[0]= (PL_Word)!_close( *sendedHere ); 
                ...
                case INDEX_READ:
                    assert( backBufferReady ==0 );
                    {
                        ...
                        int bytes;
                        if ( isText ){
                            bytes= (PL_Word)_read( fd, charBuf, count ); 
                        }
                        else{
                            bytes= (PL_Word)_read( fd, bufferToBoard+2, count*4 ); 
                        }
                        bufferToBoard[1]= errno;
                        if ( bytes != PL_Word(-1) ){
                            if ( isText ){
                ...
                case INDEX_WRITE:
                    assert( backBufferReady ==0 );
                    {
                        ...
                        if ( isText ){
                            ...
                            bytes= (PL_Word)_write( fd, charBuf, count ); 
                            bufferToBoard[0]= bytes;
                        }
                        else{
                            bytes= (PL_Word)_write( fd, (int*)ptr, count*4 ); 
                ...
                case INDEX_GETPOS:
                {
                    ...
                    int fd= *ptr++;
                    long pos= _tell( fd ); 
                    assert( sizeof(long)==sizeof(PL_Word) );
                    bufferToBoard[0]= (PL_Word)pos;
                    bufferToBoard[1]= errno;
                    backBufferReady= 2;
                    break;
                }
                ...
                case INDEX_FSTAT:
                {
                    ...
                    stsp->absorb( st );
                    bufferToBoard[1]= errno;
                    backBufferReady= 2+sizeof(struct _stat64i32_sparse)/sizeof(PL_Word);
                    break;
                }
                default:
...
//                    throw( "unknown service called" );
        ...
        if ( backBufferReady )
            if ( PL_WriteMemBlock( a, bufferToBoard, 
                                    bssSecAddr +b2hBufSize, 
                                    backBufferReady) != PL_OK )
