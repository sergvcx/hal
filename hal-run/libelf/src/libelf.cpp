// Библиотека доступа к объектным и исполняемым файлам формата ELF
// Файл реализации методов и функций библиотеки доступа к файлам формата ELF
...
Elf32_Ehdr::Elf32_Ehdr()
{
    ...
    e_phoff = Elf32_Off(0);
    e_shoff = Elf32_Off(0);
    e_flags = Elf32_Word( EM_NM640X_6403 );
...
Elf32_Ehdr::Elf32_Ehdr(Elf32_Half TargetProc)
{
	e_ident[EI_MAG0]  = ELFMAG0;
	e_ident[EI_MAG1]  = ELFMAG1;
	e_ident[EI_MAG2]  = ELFMAG2;
...
Elf32_Word
Elf32_Ehdr::convertEMachine( Elf32_Half oldEMachine, Elf32_Word oldEFlags )
{
    if ( oldEFlags ){
        assert ( oldEMachine == EM_NM640X );
        return oldEFlags;
    }
    if( oldEMachine <= EM_NM640X_NMC4FI )
	{
	    return oldEMachine;
	}
    switch( oldEMachine )
	{
		case EM_NEUROCHIP:
            return EM_NM640X_6403;
		case EM_NEUROCORE01:
            return EM_NM640X_NMC3;
		case EM_NM6405:
            return EM_NM640X_6405;
		case EM_SOC:
            return EM_NM640X_SOC;
    }
	throw (ELF_ERROR_EHDR("Unknown target processor.\n"));
    return EM_NONE;
}

void 
Elf32_Ehdr::setMachine( Elf32_Half TargetProc )
{
    e_flags = convertEMachine( TargetProc, 0 );
	e_machine = EM_NM640X;
}

Elf32_Half 
Elf32_Ehdr::getMachine() const
{
    if ( e_machine != EM_NM640X )
	    throw (ELF_ERROR_EHDR("Unknown target processor.\n"));
    return (Elf32_Half)e_flags;
}
    ...
    DeleteFile( 1 )
{
    ...
    try {
        file = new fstream;
        if( file == NULL )
            throw ELF_ERROR("Elf error - Cann't create fstream");
        ...
        if (c == ELF_C_READ)
            file->open(filename,ios::in | ios::binary);
                ...
                if (c == ELF_C_RDWR)
                    ...
                    throw ELF_ERROR_UNKNOWN("Elf error - Unknown file open command");
            }
...
ELF::ELF(char *buf, unsigned int siz, ELF_CMD c) throw (ELF_ERROR):
	ehdr(0),
	phdr(0),
	listscn(0),
	cmd(c),
	...
	DeleteFile(1)
{
	...
	try
	{
		if (!buf || !siz || c != ELF_C_READ)
			throw ELF_ERROR("Elf error - Failed create stream for buffer");

		sbuf = new strstream(buf, siz);
...
// protected constructor for use in derived class
...
ELF::~ELF() throw (ELF_ERROR)
{
    ...
    if (ehdr) delete ehdr;          ehdr = 0;
    if (listscn) delete listscn;    listscn = 0;
    if (phdr) delete[] phdr;        phdr = 0;
}
...
{
    Elf32_Shdr *sptr,*ptr1 = new Elf32_Shdr[(ehdr->e_shnum)];
    ELF_SCN *ptr = listscn;
    sptr = ptr1;
...
Elf32_Ehdr *ELF::GetEhdr(Elf32_Half TargetProc) throw (ELF_ERROR)
{
    if (ehdr) return ehdr;
    if (cmd == ELF_C_WRITE)
        throw (ELF_ERROR_EHDR("Elf error - Elf header not exist"));
    ...
    try {
        ehdr = new Elf32_Ehdr(TargetProc);
        pfile->seekg(0 + base_offset);
        pfile->read((char *)ehdr, ELF32_T_EHDR_FSIZE );

/*        ehdr->e_flags = Elf32_Ehdr::convertEMachine( ehdr->e_machine, ehdr->e_flags );
        ehdr->e_machine = EM_NM640X;*/
...
Elf32_Ehdr *ELF::NewEhdr(Elf32_Half TargetProc) throw (ELF_ERROR)
{
    if (ehdr) return ehdr;
    if (cmd != ELF_C_WRITE)
        return GetEhdr(TargetProc);
...
char *ELF::GetIdent(unsigned char *ptr) throw (ELF_ERROR)
{
    ...
    try {
        if (ptr == 0) sptr = new char[EI_NIDENT];
        else sptr = (char *)(ptr);
        if (ehdr) {
...
{
    Elf32_Shdr *sptr = new Elf32_Shdr[ehdr->e_shnum];
    ...
    for (long i = ehdr->e_shnum - 1;i >= 0; i--)
        listscn = new ELF_SCN( this, i, ELF_F_NONE,ELF_F_NONE,
            listscn, 0L, &sptr[i] );
    if( listscn == NULL )
    {
        if( sptr ) delete[] sptr;
...
//  if (shdr) delete shdr;
    try {
        ...
        sptr = new Elf32_Shdr;
        ptr->nextScn = new ELF_SCN(this,ehdr->e_shnum,
    ...
    throw (ELF_ERROR)
{
    ...
    if ( Phdr->p_filesz != 0 ) {
        ...
        if (pfile->bad())
            throw ELF_ERROR_FILE("Elf Error program segment reading returned status 'bad'");
        if (pfile->fail())
            throw ELF_ERROR_FILE("Elf Error program segment reading returned status 'fail'");
        if (pfile->eof())
            throw ELF_ERROR_FILE("Elf Error program segment reading returned status 'end of file'");
    }
...
//----------------------------------------------------------------------//
//                                                                      //
// Функции работающие с именами секций                                  //
//                                                                      //
//----------------------------------------------------------------------//

//============================================================================
//  Функция sectionFlagsFromName находит стандартные значения 
//  флагов для секций с предопределенными именами 
//=============================================================================void LINKER::CreateInitialAttributes()

Elf32_Word
ELF::sectionFlagsFromName( const char* name )
{
    switch ( name[1] ){
        case 'b':
	        if ( !strncmp( name, ".bss", 4 ))	//	имена, _начинающиеся_ с .bss
		        return SHF_ALLOC | SHF_WRITE;
            break;
        case 'c':
	        if ( !strncmp( name, ".common", 7 ))
		        return SHF_ALLOC | SHF_WRITE;
            break;
        case 'd':
            ...
            break;
        case 'f':
	        if ( !strncmp( name, ".fini", 5 ))
		        return SHF_ALLOC | SHF_EXECINSTR;
            break;
        case 'h':
	        if ( !strncmp( name, ".heap", 5 ))
		        return SHF_ALLOC | SHF_WRITE;
            break;
        case 'i':
...
//      s = scn->GetShdr();
    ...
    void *pzero = (void *) new char[strlen(sptr) + 1]; ;

    ptr = scn->NewData();
    ptr->d_size = (Elf32_Word)strlen(sptr) + 1;
    ptr->d_buf = pzero;
    ptr->d_type = ELF32_T_BYTE;
    ptr->d_align = ELF32_T_BYTE_FSIZE;
    ptr->d_version = EV_CURRENT;
...
void ELF_SCN::elf_internal_load_section(Elf_Type t) throw(ELF_ERROR)
{
    ...
    listdata->d_buf = (void *) new char[shdr->sh_size];
    listdata->d_type = t;
...
ELF_ERROR_AMBIGUITY::ELF_ERROR_AMBIGUITY(
    ...
    eeaAmName( 0 )
{
    eeaAmName = new char[ strlen(AmName) + 1];
...
{
    msg3 = 0;
    eeaAmName = new char[ strlen(ErrAm.eeaAmName) + 1];
