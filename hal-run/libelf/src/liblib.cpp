// ELF library
// Файл реализации методов и функций для работы с библиотеками объектных файлов формата ELF
    ...
    ) throw (ELF_ERROR)
{
    ...
    BlockSize += (Elf32_Word)strlen( Elf->GetName() ) + 1;   // имя файла
    ...
    for ( Elf32_Word i = 0; i < Size; i++ ) {
        ...
        BlockSize += (Elf32_Word)strlen( Name ) + 1;
        SymbolCount += 1;
    }
...
//                                      // имени символа
        ...
        Sym[ CurrSymbol ].ls_name_offset = (Elf32_Word)(ptr - Block->GetBuffer() + StringSize);
...
//        lhdr->l_lhnum = 2;
    ...
    if ( c == ELF_C_WRITE ) {
        ...
        for ( Elf32_Word i = 0; i < lhdr->l_lhnum; i++ ) {
            ...
            Libhdr[i].lib_date = (Elf32_Word)libelem_hdr[i].lib_date;
